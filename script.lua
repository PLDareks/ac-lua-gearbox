...
-- Throttle Cut And Blip -- new code to cooperate with new clutch control
		...
		if logic_gear_up > 0 then
			if logic_gear_up > 0.193 then
				data.gas = throttle 
				* interpolate(0,1, throttle, 1, 0.45) * interpolate(1,110, car.speedKmh, 0.75, 1)
				elseif logic_gear_up < 0.193 then
					data.gas = throttle 
					* interpolate(0, 0.193, logic_gear_up, 1, 0.45) * interpolate(1,110, car.speedKmh, 0.1, 1)
			else
				data.gas = throttle		
			end
		end
...
-- Shifting Locks
		...
		if data.gearUp or data.gearDown then
			shift_lock = 1 * ( interpolate(0, 1, throttle, 2, 1) * interpolate(0, 1, braking, 1, 0.5) ) - dt
			elseif kickdown == true or car.speedKmh < 5 then
				shift_lock = 0
			else
		...
		-- ac.isControllerGearUpPressed()
		-- ac.isControllerGearDownPressed()
		
		--[[if shift_paddle_lock >= 0 then
			if ac.isControllerGearUpPressed() then
				paddle_shift = true
				elseif data.gearUp == false then
					paddle_shift = false
			end		
			if ac.isControllerGearDownPressed() then
				paddle_shift = true
				elseif data.gearDown == false then
					paddle_shift = false
			end
		end]]
		
		if allow_paddle_shift == true then
			shift_paddle_lock = math.clamp(shift_paddle_lock, 0.1, 1)
			else
				shift_paddle_lock = math.clamp(shift_paddle_lock, 0, 1)
		end	
		
		if data.gearUp or data.gearDown then
			shift_paddle_lock = 1 - dt
			else
				shift_paddle_lock = shift_paddle_lock - dt
		end
...
-- Gearbox Driving Modes
	...
	-- Neutral Gear
		if ac.isKeyDown(16) and ac.isKeyDown(50) and car.speedKmh < 1 or car.speedKmh < 1 and data.requestedGearIndex == 1 
			or gearbox_valve_reverse == false and gearbox_valve_normal == false and gearbox_valve_sport == false and gearbox_valve_track == false then
...
-- Launch Control
		...
		if ac.isKeyDown(76) and ac.isKeyDown(16) and car.speedKmh < 1 and car.gear == 1 and car.tractionControlModes > 0 then
